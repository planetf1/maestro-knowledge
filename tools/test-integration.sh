...
./maestro-k vectordb list --help > /dev/null
print_success "List command help works"

# Test dry-run mode
print_status "Testing dry-run mode..."
DRY_RUN_OUTPUT=$(./maestro-k vectordb list --dry-run)
...
VERBOSE_OUTPUT=$(./maestro-k vectordb list --dry-run --verbose)
...
ENV_OUTPUT=$(./maestro-k vectordb list --verbose 2>&1 || true)
...
FLAG_OUTPUT=$(./maestro-k vectordb list --verbose --mcp-server-uri="http://localhost:9999" 2>&1 || true)
...
./maestro-k vectordb delete test_local_milvus --silent --force 2>/dev/null || true
./maestro-k vectordb delete test_remote_weaviate --silent --force 2>/dev/null || true

# Clean up any test collections that might exist in other databases
print_status "Cleaning up any existing test collections..."
for db_name in $(./maestro-k vectordb list --silent 2>/dev/null | grep -E "test_local_milvus|test_remote_weaviate" | awk '{print $1}' 2>/dev/null || true); do
    for collection in $(./maestro-k collection list --vdb="$db_name" --silent 2>/dev/null | grep -E "test_collection_.*" | awk '{print $1}' 2>/dev/null || true); do
        ./maestro-k collection delete "$collection" --vdb="$db_name" --silent --force 2>/dev/null || true
    done
done
...
cd "$CLI_DIR"
INITIAL_LIST=$(./maestro-k vectordb list --verbose)
...
CREATE_OUTPUT=$(./maestro-k vectordb create ../tests/yamls/test_local_milvus.yaml --verbose)
...
LIST_AFTER_CREATE=$(./maestro-k vectordb list --verbose)
...
CREATE_WEAVIATE_OUTPUT=$(./maestro-k vectordb create ../tests/yamls/test_remote_weaviate.yaml --verbose 2>&1 || true)
...
LIST_BOTH=$(./maestro-k vectordb list --verbose)
...
EMBEDDINGS_OUTPUT=$(./maestro-k embedding list --vdb=test_local_milvus --verbose)
...
EMBED_ALIAS_OUTPUT=$(./maestro-k embed list --vdb=test_local_milvus --verbose)
...
print_status "Testing embeddings functionality with 'embed' alias..."
VDB_EMBED_ALIAS_OUTPUT=$(./maestro-k embed list --vdb=test_local_milvus --verbose)
...
EMBEDDINGS_NONEXISTENT_OUTPUT=$(./maestro-k embedding list --vdb=non_existent_database 2>&1 || true)
...
EMBEDDINGS_MISSING_NAME_OUTPUT=$(./maestro-k embedding list 2>&1 || true)
...
COLLECTIONS_OUTPUT=$(./maestro-k collection list --vdb=test_local_milvus --verbose)
...
COLS_ALIAS_OUTPUT=$(./maestro-k coll list --vdb=test_local_milvus --verbose)
...
print_status "Testing collections functionality with 'coll' alias..."
VDB_COLS_ALIAS_OUTPUT=$(./maestro-k coll list --vdb=test_local_milvus --verbose)
...
COLLECTIONS_NONEXISTENT_OUTPUT=$(./maestro-k collection list --vdb=non_existent_database 2>&1 || true)
...
COLLECTIONS_MISSING_NAME_OUTPUT=$(./maestro-k collection list --vdb= 2>&1 || true)
...
CREATE_COLLECTION_OUTPUT=$(./maestro-k collection create --name="${COLLECTION_BASE}_basic" --vdb=test_local_milvus --verbose)
...
CREATE_COLLECTION_EMBEDDING_OUTPUT=$(./maestro-k collection create --name="${COLLECTION_BASE}_embedding" --vdb=test_local_milvus --embedding=text-embedding-3-small --verbose)
...
CREATE_COLLECTION_COL_ALIAS_OUTPUT=$(./maestro-k coll create --name="${COLLECTION_BASE}_col" --vdb=test_local_milvus --verbose)
...
CREATE_COLLECTION_VDB_COL_ALIAS_OUTPUT=$(./maestro-k coll create --name="${COLLECTION_BASE}_vdb_col" --vdb=test_local_milvus --verbose)
...
CREATE_COLLECTION_NONEXISTENT_OUTPUT=$(./maestro-k collection create --name=test_collection --vdb=non_existent_database 2>&1 || true)
...
CREATE_COLLECTION_MISSING_ARGS_OUTPUT=$(./maestro-k collection create --name=test_collection 2>&1 || true)
...
CREATE_COLLECTION_TOO_MANY_ARGS_OUTPUT=$(./maestro-k collection create --name=test_collection --vdb=test_local_milvus --invalid-flag 2>&1 || true)
...
CREATE_COLLECTION_DRY_RUN_OUTPUT=$(./maestro-k collection create --name="${COLLECTION_BASE}_dry_run" --vdb=test_local_milvus --dry-run)
...
CREATE_COLLECTION_SILENT_OUTPUT=$(./maestro-k collection create --name="${COLLECTION_BASE}_silent" --vdb=test_local_milvus --silent)
...
COLLECTIONS_AFTER_CREATE=$(./maestro-k collection list --vdb=test_local_milvus --verbose)
...
GET_COL_OUTPUT=$(./maestro-k collection list --vdb=test_local_milvus --verbose)
...
LIST_DOCS_OUTPUT=$(./maestro-k document list --vdb=test_local_milvus --collection="${COLLECTION_BASE}_basic" --verbose)
...
LIST_DOCS_DOCS_ALIAS_OUTPUT=$(./maestro-k doc list --vdb=test_local_milvus --collection="${COLLECTION_BASE}_basic" --verbose)
...
LIST_DOCS_VDB_DOCS_ALIAS_OUTPUT=$(./maestro-k doc list --vdb=test_local_milvus --collection="${COLLECTION_BASE}_basic" --verbose)
...
GET_COL_2_OUTPUT=$(./maestro-k collection list --vdb=test_local_milvus --verbose)
LIST_DOCS_2_OUTPUT=$(./maestro-k document list --vdb=test_local_milvus --collection="${COLLECTION_BASE}_embedding" --verbose)
...
LIST_DOCS_NONEXISTENT_OUTPUT=$(./maestro-k document list --vdb=test_local_milvus --collection=non_existent_collection 2>&1 || true)
...
DELETE_COLLECTION_OUTPUT=$(./maestro-k collection delete "${COLLECTION_BASE}_basic" --vdb=test_local_milvus --verbose --force)
...
DELETE_COLLECTION_COL_ALIAS_OUTPUT=$(./maestro-k coll delete "${COLLECTION_BASE}_col" --vdb=test_local_milvus --verbose --force)
...
DELETE_COLLECTION_VDB_COL_ALIAS_OUTPUT=$(./maestro-k coll delete "${COLLECTION_BASE}_vdb_col" --vdb=test_local_milvus --verbose --force)
...
DELETE_COLLECTION_DEL_ALIAS_OUTPUT=$(./maestro-k collection delete "${COLLECTION_BASE}_embedding" --vdb=test_local_milvus --verbose --force)
...
DELETE_COLLECTION_DRY_RUN_OUTPUT=$(./maestro-k collection delete "${COLLECTION_BASE}_dry_run" --vdb=test_local_milvus --dry-run)
...
DELETE_COLLECTION_SILENT_OUTPUT=$(./maestro-k collection delete "${COLLECTION_BASE}_silent" --vdb=test_local_milvus --silent --force)
...
DELETE_COLLECTION_NONEXISTENT_DB_OUTPUT=$(./maestro-k collection delete test_collection --vdb=non_existent_database --force 2>&1 || true)
...
DELETE_COLLECTION_MISSING_NAME_OUTPUT=$(./maestro-k collection delete 2>&1 || true)
...
COLLECTIONS_AFTER_DELETE=$(./maestro-k collection list --vdb=test_local_milvus --verbose)
...
DELETE_OUTPUT=$(./maestro-k vectordb delete test_local_milvus --verbose --force)
...
LIST_AFTER_DELETE=$(./maestro-k vectordb list --verbose)
if [[ "$WEAVIATE_CREATED" == "true" ]]; then
