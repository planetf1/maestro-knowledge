...
# Fixed Strategy
# Splits text into fixed-size chunks (optionally overlapping).
def _fixed_chunk(
...
# Sentence splitting
# Splits text into chunks at sentence boundaries, aiming for each chunk to be close to chunk_size.
def _sentence_chunk(
    text: str, chunk_size: int = 512, overlap: int = 0
) -> List[Dict[str, object]]:
    if chunk_size <= 0:
