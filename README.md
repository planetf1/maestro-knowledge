...
### Custom Embeddings with Milvus

When using the Milvus backend with a `custom_local` embedding model, you can configure the following environment variables:

- `CUSTOM_EMBEDDING_URL`: The URL of your custom embedding model server.
- `CUSTOM_EMBEDDING_MODEL`: The name of the model to use on the custom server.
- `CUSTOM_EMBEDDING_VECTORSIZE`: The dimension of the embedding vectors generated by your model.

For improved performance when embedding a large number of documents, you can enable batched embedding requests. This is especially useful when your custom embedding model is hosted remotely, as it reduces the number of network round-trips.

- `EMBEDDING_BATCH_SIZE`: The number of text chunks to send in a single request to the embedding model. Defaults to `32`.
...
- `EMBEDDING_BATCH_SIZE`: For Milvus, sets the number of chunks to embed in a single batch when using a custom embedding model. Defaults to `32`.
