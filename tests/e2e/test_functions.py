...
async def run_document_operations_tests(client: Any, backend_name: str) -> None:
    ...
    docs = [
        {"url": f"https://example.com/doc1", "text": f"{backend_name.title()} document one"},
        {"url": f"https://example.com/doc2", "text": f"{backend_name.title()} document two"},
    ]
    res = await client.call_tool(
        "write_documents",
        {
...
async def run_query_operations_tests(client: Any, backend_name: str) -> None:
    ...
    docs = [
        {"url": "https://example.com/ai", "text": "Artificial intelligence and machine learning"},
        {"url": "https://example.com/vector", "text": "Vector databases for semantic search"},
        {"url": f"https://example.com/{backend_name}", "text": f"{backend_name.title()} is a vector database"},
    ]
    res = await client.call_tool(
        "write_documents",
        {
...
async def run_document_retrieval_tests(client: Any, backend_name: str) -> None:
    ...
    if isinstance(res.data, list) and len(res.data) > 0:
        ...
        if doc_id:
            ...
            res = await client.call_tool(
                "get_document",
                {"input": {"db_name": db_name, "doc_id": doc_id}}
            )
            assert hasattr(res, "data")
...
async def run_bulk_operations_tests(client: Any, backend_name: str) -> None:
    ...
    docs = [
        {"url": f"https://example.com/bulk{i}", "text": f"{backend_name.title()} bulk document {i}"}
        for i in range(1, 4)
    ]
    res = await client.call_tool(
        "write_documents",
    ...
    if isinstance(res.data, list) and len(res.data) >= 2:
        ...
        if doc_ids:
            ...
            res = await client.call_tool(
                "delete_documents",
                {"input": {"db_name": db_name, "doc_ids": doc_ids}}
            )
            assert hasattr(res, "data")
...
async def run_collection_specific_tests(client: Any, backend_name: str) -> None:
    ...
    try:
        ...
        if not hasattr(res, "data"):
            pytest.fail(f"write_document_to_collection failed for {backend_name}: {res}")
        ...
        if not hasattr(res, "data"):
            pytest.fail(f"list_documents_in_collection failed for {backend_name}: {res}")
        ...
        if not hasattr(res, "data"):
            pytest.fail(f"delete_document_from_collection failed for {backend_name}: {res}")
        ...
        if "pytest" in str(type(e)) and "Skip" in str(type(e)):
            ...
            pytest.fail(f"Exception in collection-specific test for {backend_name}: {e}")
...
async def run_full_flow_test(client: Any, backend_name: str) -> None:
    ...
    try:
        ...
        if not collection_created:
            pytest.skip(f"Could not create collection for {backend_name} after retries: {res}")

        # Write documents
        docs = [
            {"url": f"https://example.com/{backend_name}1", "text": f"hello {backend_name} vector world"},
            {"url": f"https://example.com/{backend_name}2", "text": f"maestro knowledge {backend_name}"},
        ]
        res = await client.call_tool(
            "write_documents",
            {
